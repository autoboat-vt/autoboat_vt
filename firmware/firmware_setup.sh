...
if [ ! -d "$REPOSITORY_ROOT/firmware/dependencies/pico-sdk" ]; then
  git clone --recurse-submodules https://github.com/raspberrypi/pico-sdk.git $REPOSITORY_ROOT/firmware/dependencies/pico-sdk
fi
...
if [ ! -d "$REPOSITORY_ROOT/firmware/dependencies/picotool" ]; then
  git clone https://github.com/raspberrypi/picotool $REPOSITORY_ROOT/firmware/dependencies/picotool
fi

cd $REPOSITORY_ROOT/firmware/dependencies/picotool
export PICOTOOL_PATH=$REPOSITORY_ROOT/firmware/dependencies/picotool
...
if [ ! -d "$REPOSITORY_ROOT/firmware/dependencies/micro_ros_agent/src/micro_ros_setup" ]; then
  ...
  cd $REPOSITORY_ROOT/firmware/dependencies/micro_ros_agent/src/micro_ros_setup
...
sudo mv $REPOSITORY_ROOT/firmware/dependencies/** /opt/autoboat/firmware_dependencies
rm -rf $REPOSITORY_ROOT/firmware/dependencies

# Write sources and aliases to bashrc file
{
  echo "unset ROS_DOMAIN_ID"
  echo "export PICO_SDK_PATH=/opt/autoboat/firmware_dependencies/pico-sdk"
  echo "export PICO_MICROROS_SDK_PATH=/opt/autoboat/firmware_dependencies/micro_ros_raspberrypi_pico_sdk"
  echo "export PICOTOOL_PATH=/opt/autoboat/firmware_dependencies/picotool"
  echo "source /opt/autoboat/firmware_dependencies/micro_ros_agent/install/local_setup.bash"

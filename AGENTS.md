...
- **Ground station logging**: all ground station Python code uses the stdlib `logging` module via `get_logger(__name__)` from `utils.logger`. Logs flow to the `ConsoleOutputWidget` (via `QtConsoleHandler`) and to a rotating file at `app_data/git_ignore/logs/ground_station.log` (5MB, 5 backups). Never use `print()` in ground station code - use `logger.info`/`logger.warning`/`logger.error`. The formatter emits `[Info]`/`[Warning]`/`[Error]` prefixes so the existing `ConsoleHighlighter` regex keeps working. `ruff.toml` ignores `T201` (print) globally, so the ban is by convention, not linter-enforced.

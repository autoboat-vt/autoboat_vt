"""Audio playback for the Snake easter egg."""
...
class SnakeAudio:
    """
    Manage music and sound effects for Snake.
    ...
    All file paths are resolved from ``constants.SNAKE_MUSIC_DIR`` and
    ``constants.SNAKE_SFX_DIR``. If the files don't exist, every method
    silently no-ops so the game still works without audio.
    """
    ...
    def __init__(self) -> None:
        ...
        self._sfx_eat_food: QSoundEffect | None = None
    ...
    def _load_tracks(self) -> None:
        """Discover music tracks in the snake music directory."""

        music_dir = constants.SNAKE_MUSIC_DIR
    ...
    def _load_sfx(self) -> None:
        ...
        self._sfx_eat_food = self._make_sound_effect(constants.SNAKE_SFX_DIR / "eat_food.wav")
        self._sfx_game_over = self._make_sound_effect(constants.SNAKE_SFX_DIR / "game_over.wav")
    ...
    def play_eat_food(self) -> None:
        """Play the eat-food sound effect."""

        if self._sfx_enabled and self._sfx_eat_food is not None:
            self._sfx_eat_food.play()
    ...
    def cleanup(self) -> None:
        ...
        self.stop_music()
        for sfx in (self._sfx_eat_food, self._sfx_game_over):
            if sfx is not None:
                sfx.stop()

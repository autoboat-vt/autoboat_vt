...
class TempPowerStatsPublisher(Node):
    ...
    def timer_callback(self):
        ...
        jetson_stats_message.soc_temperature_celcius = (
            float(jetson_stats["Temp soc0"]) + float(jetson_stats["Temp soc1"]) + float(jetson_stats["Temp soc2"])
        ) / 3

        jetson_stats_message.soc_power_usage_watts = float(jetson_stats["Power VDD_SOC"]) / 1000.0
        jetson_stats_message.total_power_usage_watts = float(jetson_stats["Power TOT"]) / 1000.0
        ...
        for key, value in jetson_stats.items():
            if "CPU" in key:
                try:
                    int(
                        key.removeprefix("CPU")
                    )  # eg. CPU1, CPU2, etc. If it is not in this form then it isn't a cpu utilization percent
                except:
                    continue

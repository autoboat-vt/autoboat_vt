...
SailboatStates SailboatAutopilot::get_current_waypoint_mission_state() {
    return current_waypoint_mission_state;
}



std::pair<float, float> SailboatAutopilot::step(
    ...
    float rc_rudder_control,
    float rc_sail_control
) {
...
std::pair<float, float> SailboatAutopilot::run_waypoint_mission_step(Position current_position, std::array<float, 2> global_velocity_vector, float heading, std::array<float, 2> apparent_wind_vector) {

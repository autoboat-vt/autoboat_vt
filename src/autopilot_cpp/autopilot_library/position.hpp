...
public:
    TelemetryNode(double longitude_, double latitude_) {
    ...
    void set_local_coordinates(double local_x, double local_y, double reference_longitude, double reference_latitude) {
        std::tuple<double> lla_vector = navpy.ned2lla(std::vector<double>({local_y, local_x, 0}), reference_latitude, reference_longitude, 0);
        latitude = lla_vector[0];
        longitude = lla_vector[1];
    }


    std::tuple get_local_coordinates(double reference_longitude, double reference_latitude) {
        std::tuple<double> ned_vector = lla2ned(latitude, longitude, 0, reference_latitude, reference_longitude, 0);
        return std::tuple<double>(ned_vector[1], ned_vector[0]);
    }

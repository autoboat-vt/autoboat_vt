...
class NTRIPClient:
    ...
    def __init__(
        ...
        if username is not None and password is not None:
            self._basic_credentials = base64.b64encode("{}:{}".format(username, password).encode("utf-8")).decode("utf-8")
        else:
        ...
        self.rtcm_parser = RTCMParser(logerr=logerr, logwarn=logwarn, loginfo=loginfo, logdebug=logdebug)
        self.nmea_parser = NMEAParser(logerr=logerr, logwarn=logwarn, loginfo=loginfo, logdebug=logdebug)
    ...
    def connect(self):
        ...
        if known_error or not self._connected:
            self._logerr("Invalid response received from http://{}:{}/{}".format(self._host, self._port, self._mountpoint))
            self._logerr("Response: {}".format(response))
            return False
        else:
            self._loginfo("Connected to http://{}:{}/{}".format(self._host, self._port, self._mountpoint))
            return True

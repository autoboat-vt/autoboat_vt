...
class RCPublisher(Node):
    ...
    def save_frame(self, frame: crsf_frame, status: PacketValidationStatus):
        ...
        if frame.header.type != PacketsTypes.RC_CHANNELS_PACKED or status.value != 1:
            return
    ...
    def parse_toggle(self, toggle_state):
        ...
        if toggle_state <= 500:
            return 0
        elif toggle_state <= 1500:
            return 1
        elif toggle_state <= 2000:
            return 2
        else:
    ...
    def parse_multiplexed_buttons(self, button_state):
        ...
        if button_state < 500:
            return False, False
        elif button_state >= 500 and button_state < 1000:
            return True, False
        elif button_state >= 1000 and button_state < 1500:
            return False, True
        elif button_state >= 1500:
            return True, True
    ...
    def process_raw_channels(self, raw_channel_array: list) -> RCData:
        ...
        if toggle_b == -1 or toggle_c == -1 or toggle_e == -1 or toggle_f == -1:
            return None
    ...
    def timer_callback(self):
        ...
        if not self.crsf_frame_rc_data:
            return

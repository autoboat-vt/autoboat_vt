...
void application_init(rcl_allocator_t* allocator, rclc_support_t* support, rclc_executor_t* executor)
{
  ...
  // -----------------------------------------------------
  RCCHECK(rclc_subscription_init_default(&zero_rudder_encoder_subscriber, &microros_node,
                                         ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool), "/zero_rudder_encoder"));
  RCCHECK(rclc_executor_add_subscription(executor, &zero_rudder_encoder_subscriber, &zero_rudder_encoder_msg,
                                         &zero_rudder_encoder_callback, ON_NEW_DATA));
#if BOAT_MODE == Lumpy
  RCCHECK(rclc_subscription_init_default(&zero_winch_encoder_subscriber, &microros_node,
                                         ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool), "/zero_winch_encoder"));
  RCCHECK(rclc_executor_add_subscription(executor, &zero_winch_encoder_subscriber, &zero_winch_encoder_msg,
                                         &zero_winch_encoder_callback, ON_NEW_DATA));
#endif
...
// -----------------------------------------------------
  ...
  RCCHECK(rclc_subscription_init_default(&should_propeller_motor_be_powered_subscriber, &microros_node,
                                         ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool),
                                         "/should_propeller_motor_be_powered"));
  RCCHECK(rclc_executor_add_subscription(executor, &should_propeller_motor_be_powered_subscriber,
                                         &should_propeller_motor_be_powered_msg,
                                         &should_propeller_motor_be_powered_callback, ON_NEW_DATA));
#endif

  // -----------------------------------------------------
  // INITIALIZE THE CURRENT RUDDER PUBLISHERS AND SUBSCRIBERS
  // -----------------------------------------------------
  RCCHECK(rclc_publisher_init_default(&current_rudder_angle_publisher, &microros_node,
                                      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32), "/current_rudder_angle"));
  RCCHECK(rclc_publisher_init_default(&current_rudder_motor_angle_publisher, &microros_node,
                                      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32),
                                      "/current_rudder_motor_angle"));
  RCCHECK(rclc_subscription_init_best_effort(&desired_rudder_angle_subscriber, &microros_node,
                                             ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32),
                                             "/desired_rudder_angle"));
  RCCHECK(rclc_executor_add_subscription(executor, &desired_rudder_angle_subscriber, &desired_rudder_angle_msg,
                                         &desired_rudder_angle_received_callback, ON_NEW_DATA));

// -----------------------------------------------------
// INITIALIZE THE CURRENT SAIL PUBLISHERS AND SUBSCRIBERS (ONLY FOR LUMPY)
// -----------------------------------------------------
#if BOAT_MODE == Lumpy
  RCCHECK(rclc_publisher_init_default(&current_sail_angle_publisher, &microros_node,
                                      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32), "/current_sail_angle"));
  RCCHECK(rclc_publisher_init_default(&current_winch_angle_publisher, &microros_node,
                                      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32), "/current_winch_angle"));
  RCCHECK(rclc_subscription_init_best_effort(&desired_winch_angle_subscriber, &microros_node,
                                             ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32),
                                             "/desired_sail_angle"));
  RCCHECK(rclc_executor_add_subscription(executor, &desired_winch_angle_subscriber, &desired_winch_angle_msg,
                                         &desired_sail_angle_received_callback, ON_NEW_DATA));
#endif

  // -----------------------------------------------------
  // INITIALIZE THE CURRENT HEADING PUBLISHER
  // -----------------------------------------------------
  RCCHECK(rclc_publisher_init_default(&compass_angle_publisher, &microros_node,
                                      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32), "/heading"));
  ...
  drv8711_init(&rudderStepperMotorDriver, SPI_PORT, RUDDER_MOTOR_CS_PIN, RUDDER_MOTOR_SLEEP_PIN, AutoMixed,
               RUDDER_MICROSTEP, MAX_RUDDER_CURRENT);

  cmps14_init(&compass, I2C_PORT, 0x60);

#if BOAT_MODE == Lumpy
  AMT22_init(&winchEncoder, WINCH_ENCODER_CS_PIN, SPI_PORT);
  drv8711_init(&winchStepperMotorDriver, SPI_PORT, WINCH_MOTOR_CS_PIN, WINCH_MOTOR_SLEEP_PIN, AutoMixed,
               WINCH_MICROSTEP, MAX_WINCH_CURRENT);
#endif

#if BOAT_MODE == Theseus
  initialize_contactor_driver(CONTACTOR_DRIVER_SEL0_PIN, CONTACTOR_DRIVER_PWM_PIN, CONTACTOR_DRIVER_IN_A_PIN,
                              CONTACTOR_DRIVER_IN_B_PIN);
#endif
...
void application_loop()
{
  ...
  compass_angle_msg.data = rudder_error;  // fmod((-cmps14_getBearing(&compass) / 10.0 + COMPASS_OFFSET + 360), 360.0);

//Orignal naming, cmps14_compass_library.h
//Spell Library correctly
//goal is to created a object orientated system
//Method names have removed cmps14_ prefix due to now being called with cmps14.methodName();
...
public:
    ...
    cmps14(i2c_inst_t* port, uint8_t address) 
        : bearing(0), pitch(0), roll(0), i2cAddress(address), i2cPort(*port) {} 
    ...
    int readBytes(uint8_t reg, uint8_t *buffer, uint8_t length){
        ...
        int result = i2c_read_blocking(&i2cPort, i2cAddress, buffer, length, false);
        return result;
    }

    int16_t getBearing() {
        uint8_t buffer[2];
        if (readBytes(BEARING_Register, buffer, 2) != 2) {
        return -1;
        }
...
};//end cmps14        // WHY TF IS THERE A SEMICOLON O U T S I D E THE PARENTHESIS BRACKET idk what they called

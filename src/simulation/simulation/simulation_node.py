...
def generate_wind(_) -> np.ndarray[np.float64]:
    ...
    return np.array([np.cos(np.deg2rad(-90)) + random.random() * 0.0, np.sin(np.deg2rad(-90)) + random.random() * 0.0])
...
class SimulationNode(Node):
    ...
    def __init__(self):
        ...
        self.apparent_wind_vector = Vector3(x=1.0)
        self.true_wind_vector = Vector3(x=1.0)
    ...
    def should_terminate_callback(self, should_terminate_callback_message: Bool):
        if should_terminate_callback_message.data == False:
            return
    ...
    def publish_observation_data(self, observation: Observation) -> None:
        ...
        position = Vector3(
            x=observation["p_boat"][0].item(), y=observation["p_boat"][1].item(), z=-observation["p_boat"][2].item()
        )
        ...
        if position.x == 0.0 and position.y == 0.0:
            ...
            latitude, longitude, _ = navpy.ned2lla([position.y, position.x, position.z], lat_ref=0.0, lon_ref=0.0, alt_ref=0)
        ...
        if (
            np.isnan(observation["dt_p_boat"][0])
            or np.isnan(observation["dt_p_boat"][1])
            or np.isnan(observation["dt_p_boat"][2])
        ):
            print("WARNING: VELOCITY IS NAN")
            global_velocity_vector = Vector3()
        else:
            global_velocity_vector = Vector3(
                x=observation["dt_p_boat"][0].item(), y=observation["dt_p_boat"][1].item(), z=observation["dt_p_boat"][2].item()
            )
        ...
        true_wind_speed, global_true_wind_angle = cartesian_vector_to_polar(
            observation["wind"][0].item(), observation["wind"][1].item()
        )

        true_wind_angle = global_true_wind_angle - heading_angle.data
        self.true_wind_vector = Vector3(
            x=(true_wind_speed * np.cos(np.deg2rad(true_wind_angle))), y=(true_wind_speed * np.sin(np.deg2rad(true_wind_angle)))
        )
        ...
        self.apparent_wind_vector = Vector3(
            x=(self.true_wind_vector.x - local_velocity_vector[0]), y=(self.true_wind_vector.y - local_velocity_vector[1])
        )

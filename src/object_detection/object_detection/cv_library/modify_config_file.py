import os
import re
import sys

if len(sys.argv) < 3:
    print("Usage: python modify_config_file.py <name_of_model> <yolo_version>")
    sys.exit(1)

# if not sys.argv[1].endswith('.onnx'):
#     print("Error: ONNX file must end with .onnx")
#     sys.exit(1)

YOLO_VER = sys.argv[2]
FP_VER = 16
IS_DEV_CONTAINER = re.search("/home/ws", os.getcwd()) is not None
PATH_TO_SRC_DIR = "/home/ws/src" if IS_DEV_CONTAINER else f"{os.path.expanduser('~')}/autoboat_vt/src"
CONFIG_FILE = f'{PATH_TO_SRC_DIR}/object_detection/object_detection/config/yolo{YOLO_VER}_config.yaml'
model_name = sys.argv[1]

with open(CONFIG_FILE, "r") as file:
    content = file.read()
    split_content = content.split("\n\n")
    onnx_section = split_content[1]
    engine_section = split_content[2]
    labels_section = split_content[3]
    properties = split_content[4].split('\n')
    BATCH_SIZE = properties[1].split(': ')[-1].split(' ')[0]
    onnx_lines = onnx_section.split('\n')
    engine_lines = engine_section.split('\n')
    labels_lines = labels_section.split('\n')
    network_mode = int(properties[2].split(': ')[-1].split(' ')[0])
    match network_mode:
        case 0:
            quantize = "fp32"
        case 1:
            quantize = "int8"
        case 2:
            quantize = "fp16"
        case _:
            print(f"Unknown network mode {network_mode}. Exiting.")
            sys.exit(1)

    found_onnx_entry = False
    for i in range(len(onnx_lines)):
        if onnx_lines[i].strip().startswith("onnx-file: "):
            onnx_lines[i] = "    # " + onnx_lines[i].strip()
        if onnx_lines[i].split(': ')[-1] == f"./yolo{YOLO_VER}/onnx_files/{model_name}.onnx":
            onnx_lines[i] = f"    {onnx_lines[i].strip()[2:]}" # uncomment line so the model can be used
            found_onnx_entry = True
    if not found_onnx_entry:
        onnx_lines.append(f"    onnx-file: ./yolo{YOLO_VER}/onnx_files/{model_name}.onnx")

    found_engine_entry = False
    for i in range(len(engine_lines)):
        if engine_lines[i].strip().startswith("model-engine-file: "):
            engine_lines[i] = "    # " + engine_lines[i].strip()
        if engine_lines[i].split(': ')[-1] == f"./yolo{YOLO_VER}/engine_files/{model_name}_model_b{BATCH_SIZE}_gpu0_{quantize}.engine":
            engine_lines[i] = f"    {engine_lines[i].strip()[2:]}" # uncomment line so the model can be used
            found_engine_entry = True
    if not found_engine_entry:
        engine_lines.append(f"    model-engine-file: ./yolo{YOLO_VER}/engine_files/{model_name}_model_b{BATCH_SIZE}_gpu0_{quantize}.engine")
    found_labels_entry = False
    for i in range(len(labels_lines)):
        if labels_lines[i].strip().startswith('labelfile-path: '):
            labels_lines[i] = "    # " + labels_lines[i].strip()
        if labels_lines[i].split(': ')[-1] == f"./yolo{YOLO_VER}/label_files/{model_name}_labels.txt":
            labels_lines[i] = f"    {labels_lines[i].strip()[2:]}" # uncomment line so the model can be used
            found_labels_entry = True
    if not found_labels_entry:
        labels_lines.append(f"    labelfile-path: ./yolo{YOLO_VER}/label_files/{model_name}_labels.txt")
    
    onnx_content = "\n".join(onnx_lines)
    engine_content = "\n".join(engine_lines)
    labels_content = "\n".join(labels_lines)

    split_content[1] = onnx_content
    split_content[2] = engine_content
    split_content[3] = labels_content

    content = "\n\n".join(split_content)

with open(CONFIG_FILE, "w") as file:
    file.write(content)
